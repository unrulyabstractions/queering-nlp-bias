...
class TestClassifyArm:
    ...
    def test_classify_twig(self):
        assert classify_arm("twig_b1_1") == ArmKind.TWIG
        assert classify_arm("twig_b1_2") == ArmKind.TWIG
        assert classify_arm("twig_b2_1") == ArmKind.TWIG
...
class TestIsTwig:
    ...
    def test_twig_returns_true(self):
        assert is_twig("twig_b1_1") is True
        assert is_twig("twig_b2_3") is True
...
class TestGetParentBranch:
    ...
    def test_twig_returns_parent(self):
        assert get_parent_branch("twig_b1_1") == "branch_1"
        assert get_parent_branch("twig_b3_2") == "branch_3"
...
class TestGetTwigIndex:
    ...
    def test_twig_returns_index(self):
        assert get_twig_index("twig_b1_1") == 1
        assert get_twig_index("twig_b2_5") == 5
...
class TestGetBranchIndex:
    ...
    def test_twig_returns_branch_index(self):
        assert get_branch_index("twig_b1_1") == 1
        assert get_branch_index("twig_b3_2") == 3
...
class TestIsBaselineArm:
    ...
    def test_twig_not_baseline(self):
        assert is_baseline_arm("twig_b1_1") is False
...
class TestGetArmSortKey:
    ...
    def test_twigs_sort_after_parent_branch(self):
        ...
        twig_key = get_arm_sort_key("twig_b1_1")
...
class TestSortArmNames:
    ...
    def test_sorts_correctly(self):
        names = ["branch_2", "root", "twig_b1_1", "trunk", "branch_1"]
        result = sort_arm_names(names)
        assert result == ["root", "trunk", "branch_1", "twig_b1_1", "branch_2"]
...
class TestGetArmAncestry:
    ...
    def test_twig_ancestry(self):
        assert get_arm_ancestry("twig_b2_1") == ["root", "trunk", "branch_2", "twig_b2_1"]
...
class TestGetDownstreamArms:
    ...
    def test_branch_has_own_twigs_downstream(self):
        all_arms = ["trunk", "branch_1", "branch_2", "twig_b1_1", "twig_b2_1"]
        downstream = get_downstream_arms("branch_1", all_arms)
        assert "twig_b1_1" in downstream
        assert "twig_b2_1" not in downstream  # Different branch

    def test_twig_has_nothing_downstream(self):
        all_arms = ["trunk", "branch_1", "twig_b1_1", "twig_b1_2"]
        downstream = get_downstream_arms("twig_b1_1", all_arms)
        assert downstream == []
