...
The generation and scoring JSONs are auto-inferred from the estimation filename
...
from src.generation.generation_types import OutputPaths
from src.viz import visualize_result


def infer_method_from_path(est_path: Path) -> str:
    """Extract generation method name from estimation filename.

    Filename pattern: est_<method>_<rest...>.json
    Example: est_simple-sampling_disclosure_queer.json -> simple-sampling
    """
    stem = est_path.stem  # est_simple-sampling_disclosure_queer
    parts = stem.split("_")
    # parts[0] = "est", parts[1] = method name (may contain hyphens, not underscores)
    if len(parts) < 2:
        msg = f"Cannot infer method from filename: {est_path.name}"
        raise ValueError(msg)
    return parts[1]


def infer_score_path(est_path: Path) -> Path:
    """Infer scoring JSON path from estimation filename.

    est_<name>.json -> out/score_<name>.json
    """
...
def main() -> None:
    ...
    parser.add_argument("estimation", help="Path to estimation output JSON (out/est_*.json)")
    parser.add_argument(
        "--generation",
        help="Path to generation JSON (out/gen_*.json). Auto-inferred if omitted.",
    )
    parser.add_argument(
        "--scoring",
        help="Path to scoring JSON (out/score_*.json). Auto-inferred if omitted.",
    )
    parser.add_argument(
        "--output-dir",
        default="out/viz",
        help="Directory to write plots into (default: out/viz)",
    )
    ...
    created = visualize_result(result, output_dir=Path(args.output_dir))

    if created:
        print(f"Saved {len(created)} plots to {args.output_dir}/{method}/")
    else:
